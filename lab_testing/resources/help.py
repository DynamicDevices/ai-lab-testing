    ...
    return {
        ...
        "tools": {
            ...
            "file_transfer": {
                ...
                "copy_files_to_device_parallel": "Copy multiple files to remote device in parallel using multiplexed SSH connections. Much faster than copying files sequentially - all transfers share the same SSH connection (eliminates connection overhead). Example: copy_files_to_device_parallel('imx93_board', [['/local/app.bin', '/remote/app.bin'], ['/local/config.json', '/remote/config.json']], max_workers=5). Best practice: Use this when deploying applications with multiple binaries/configs, or when rsync is not available on device. All files transfer simultaneously, sharing the same SSH connection for maximum speed.",
            },
        },
        "resources": {
        ...
        "troubleshooting": {
            ...
            "file_transfer_fails": [
                ...
                "For large files: Transfer may take time - check network connection and VPN stability",
        ...
        "best_practices": [
            ...
            "  - Multiple files (<10): Use copy_files_to_device_parallel (fastest, no rsync needed)",
            "  - Large directory: Try sync_directory_to_device first (requires rsync), fallback to copy_files_to_device_parallel",
            "  - All transfers use multiplexed SSH (ControlMaster) - first transfer establishes connection, subsequent transfers reuse it",
            ...
            "  - For parallel transfers, max_workers=5 is optimal (adjust based on network/device capacity)",
            "",
