    ...
    return {
        ...
        "tools": {
            ...
            "ota_management": {
                ...
                "list_containers": "List Docker containers on device. Supports both Foundries devices (via VPN IP) and local config devices. Automatically falls back to VPN server if direct connection fails (device_id)",
                "get_container_logs": "Get container logs from device. Supports Foundries and local devices. Useful for debugging. Automatically falls back to VPN server if direct connection fails (device_id, container_name, tail=100, follow=false, timestamps=false)",
                "restart_container": "Restart a container on device. Supports Foundries and local devices. Useful for applying config changes. Automatically falls back to VPN server if direct connection fails (device_id, container_name)",
                "start_container": "Start a stopped container. Supports Foundries and local devices. Automatically falls back to VPN server if direct connection fails (device_id, container_name)",
                "stop_container": "Stop a running container. Supports Foundries and local devices. Automatically falls back to VPN server if direct connection fails (device_id, container_name)",
                "exec_container": "Execute command inside a running container (docker exec). Supports Foundries and local devices. Automatically falls back to VPN server if direct connection fails (device_id, container_name, command)",
                "inspect_container": "Inspect container (get detailed info: config, state, network). Supports Foundries and local devices. Automatically falls back to VPN server if direct connection fails (device_id, container_name)",
                "get_container_stats": "Get container resource usage (CPU, memory, network). Supports Foundries and local devices. Automatically falls back to VPN server if direct connection fails (device_id, container_name)",
        ...
        "common_workflows": {
            ...
            "container_debugging": [
                ...
                "5. Execute command in container: 'exec_container(device_id, container_name, 'command')'",
                "6. Start/stop containers: 'start_container(device_id, container_name)' or 'stop_container(device_id, container_name)'",
                "7. Restart if needed: 'restart_container(device_id, container_name)'",
                "Note: All container tools support both Foundries devices (via VPN IP) and local config devices",
                "Note: All tools automatically fall back to VPN server if direct connection fails or times out",
            ],
            "low_power_analysis": [
